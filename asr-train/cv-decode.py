...
if __name__ == "__main__":
    ...
    cv_csv_file = f'./cv-valid-dev-with-generated-text.csv'
    df = pd.read_csv(cv_csv_file)
    df['exact_filename'] = df['filename'].str.split('/').str[-1]
    df['finetuned_generated_text'] = ''
    ...
    for file_path in files:
        ...
        df.loc[df['exact_filename'] == current_exact_filename,
               'finetuned_generated_text'] = transcription

    df = df.drop('exact_filename', axis=1)

    output_csv = f'./{dataset_split}-with-finetuned-generated-text.csv'
